    ...
    private static func makeWeeks(
        ...
        for result in sortedResults {
            ...
            if let configuredPhase {
                remainPointsPerPick = max(configuredPhase.remainPointsPerCorrectPick ?? 0, 0)
                votedOutPointsPerPick = max(configuredPhase.votedOutPointsPerCorrectPick ?? 0, 0)
                immunityPointsPerPick = max(configuredPhase.immunityPointsPerCorrectPick ?? 0, 0)
            } else {
